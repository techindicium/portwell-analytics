...
| :- | :- | :- | :- | :- |
| `ISSUE-30` | No list of who consumes which metric | metrics | Sofia Marques | "We changed self-service and found out who cared when a customer asked." See `docs/incidents/INCIDENT-03.md`. |
...
| `ISSUE-32` | Self-service reads 0.0 for non-pilot accounts | marts | Declan Byrne | "An account that never had the portal shows zero, which reads as failure. It should be not applicable." |
...
| `ISSUE-39` | No lineage for derived fields | staging | Declan Byrne | `is_self_served` is derived in `stg_suggestions.sql` and nothing records what it came from. |
